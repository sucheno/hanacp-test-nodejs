...
var tasks = [
    ...
    mysql3, myexecute, myresults,
    mysql4, myexecute, myresults,
    mydisco
	];
...
function mysql3(cb) {
	var sql = 'insert into "fs-mta-db::tables.MyTable" values (NOW())';
    cb(null, sql);
}

function mysql4(cb) {
	var sql = 'select * from "fs-mta-db::tables.MyTable"';
    cb(null, sql);
}
